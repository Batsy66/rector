...
Процедура ВыполнитьКоманду(Знач КомандаПриложения) Экспорт
	...
	// подготавливаем файлы обновлений
		// 1. если все есть, то начинаем выгрузку (пока только эта)
		// 2. если нет файла текущей поставки, то выгружаем 
	// начинаем выгружать 

	// выгружаем исходники 1с текущая
	// создаём ветку dev
	// создаём ветку base1c
	ВыгрузитьИсходникиТекущейВерсииКонфигурацииВендора(ПараметрыОбновления, ГитРепозитарий, ОсновнойПутьВыгрузки);

	// переходим на ветку dev
	// выгружаем конфигурацию доработанную
	// мержим develop в master
	ВыгрузитьИсходникиДоработаннойКонфигурации(ПараметрыОбновления, ГитРепозитарий, ОсновнойПутьВыгрузки);

	// переходим на ветку base1c
	// выгружаем конфигурацию 1с новая
	ВыгрузитьИсходникиНовойВерсииКонфигурацииВендора(ПараметрыОбновления, ГитРепозитарий, ОсновнойПутьВыгрузки);

	// мержим base1c в dev
	Гит_СлитьВетки(ГитРепозитарий, "base1c", "develop");
...
Процедура ВыгрузитьИсходникиТекущейВерсииКонфигурацииВендора(ПараметрыОбновления, ГитРепозитарий, ОсновнойПутьВыгрузки)
	...
	Если НЕ ПараметрыОбновления.Базовая.Путь = Неопределено Тогда
		...
		ДатаКоммита = '20220101';
		МенеджерСинхронизации.СохранитьИзмененияGit(Комментарий, ИмяКоммитера, ПочтаКоммитера, ДатаКоммита);
		
		Лог.Информация("В master выгружена текущая версия поставщика");
		
		Гит_СоздатьВетку(ГитРепозитарий, "base1c");
		Гит_СоздатьВетку(ГитРепозитарий, "develop");
	КонецЕсли;

КонецПроцедуры // ВыгрузитьИсходникиТекущейВерсииКонфигурацииВендора()


Процедура ВыгрузитьИсходникиДоработаннойКонфигурации(ПараметрыОбновления, ГитРепозитарий, ОсновнойПутьВыгрузки)

	Гит_ПерейтиВВетку(ГитРепозитарий, "develop", Истина);
	...
	Комментарий = "Загружена доработанная конфигурация";
	ИмяКоммитера = "ТСМ";
	ПочтаКоммитера = "tsm@tsm-msk.ru";
	ДатаКоммита = '20220102';
	МенеджерСинхронизации.СохранитьИзмененияGit(Комментарий, ИмяКоммитера, ПочтаКоммитера, ДатаКоммита);

	Лог.Информация("В develop зафиксирована текущая версия конфигурации");
	// зафиксируем наши изменения в мастере
	
	Гит_СлитьВетки(ГитРепозитарий, "develop", "master");

	Лог.Информация("В master загружены все изменения develop");

КонецПроцедуры // ВыгрузитьИсходникиДоработаннойКонфигурации()


Процедура ВыгрузитьИсходникиНовойВерсииКонфигурацииВендора(ПараметрыОбновления, ГитРепозитарий, ОсновнойПутьВыгрузки)
	...
	ИмяКоммитера = "1c";
	ПочтаКоммитера = "1c@1c.ru";
	ДатаКоммита = '20220103';
	МенеджерСинхронизации.СохранитьИзмененияGit(Комментарий, ИмяКоммитера, ПочтаКоммитера, ДатаКоммита);

	Лог.Информация("Загружена новая версия конфигурации в base1c");

КонецПроцедуры //ВыгрузитьИсходникиНовойВерсииКонфигурацииВендора()
...
Процедура Гит_СоздатьВетку(ГитРепозитарий, Знач Ветка)

	ВеткаСуществует = Гит_ВеткаСуществует(ГитРепозитарий, Ветка);

	Если Не ВеткаСуществует Тогда
		Лог.Информация("Создаём новую ветку ""%1"" в репозитарии ""%2""",
						Ветка,
						ГитРепозитарий.ПолучитьРабочийКаталог());
		ГитРепозитарий.СоздатьВетку(Ветка);
	КонецЕсли;

КонецПроцедуры

Процедура Гит_СлитьВетки(ГитРепозитарий, ВеткаИсточник, ВеткаПриемник)
	Гит_ПерейтиВВетку(ГитРепозитарий, ВеткаПриемник, , Истина);
	ПараметрыКоманды = Новый Массив();
	ПараметрыКоманды.Добавить("merge");
	ПараметрыКоманды.Добавить(ВеткаИсточник);
	Лог.Информация("Мержим изменения из %1", ВеткаИсточник);
